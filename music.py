...
class Music(commands.Cog):
    ...
    async def queue_cleaner(self):
        now = dt.now()
        ids = reversed(tuple(i for i, queue in queues.items() if print((now - queue.last_played).total_seconds()) or not queue.vc.is_playing() and (now - queue.last_played).total_seconds() > 300))
        for i in ids:
    ...
    async def play(self, ctx, *, search):
        if is_url_playlist(search):
            ...
            if search.startswith('https://open.spotify.com/track/'):
                ...
                search = f'{track["name"]} {track["artists"][0]["name"]} {track["album"]["name"]} "auto-generated by YouTube"'
