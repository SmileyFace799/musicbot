...
class Music(commands.Cog):
    ...
    @commands.command(
        brief='Play a song',
        help='Plays a song or in a voice channel. Can either be a url or a YouTube search. The bot will join automatically upon playing. Put your search in italics *like this* to search songs only. This can also be used as a shortcut for `playlist quickplay` to play external playlists. Currently supports: YouTube, Spotify',
        usage='play [search | song url | playlist url]'
    )
    async def play(self, ctx, *, search):
        if check_urltype(search, playlist_urls):
            ...
            if search.startswith('https://open.spotify.com/track/'):
                ...
                search = f'*{track["name"]} {" ".join(artist["name"] for artist in track["artists"])}*'
    ...
    @commands.command(
        ...
        help='Does a search based on search query, returns search results for user to then pick a result. The bot will join automatically upon picking a search option. Put your search in italics *like this* to search songs only. Currently supports: YouTube',
        usage='search [search]'
    )
    ...
    @commands.command(
        ...
        help='Shows the bot\'s song queue, if the queue is shuffled, if the queue is repeating, and if the current song is repeating',
        usage='queue \\*[page]\nque \\*[page]\nq \\*[page]',
        aliases=['que', 'q']
    )
